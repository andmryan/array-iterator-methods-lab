...
let inventorNames = inventors.map(iName => ({first: iName.first, last: iName.last}))
...
let sortByBirthYear = inventors.sort((oldest, youngest) => oldest.year - youngest.year)
console.log(sortByBirthYear)

// Check your return value:
console.log('---------------------------------Exercise 3--------------------------------')
console.log('Exercise 3 My Result: ', sortByBirthYear)
...
let firstLast = people.map(fnln => {
  let [last, first] = fnln.split(', ')
  return `${first} ${last}`;
});
...
let isAdultPresent = devs.some(isAdult => {
  const age = 2024 - isAdult.year
  return age >= 18
})
...
let isEveryone19OrOlder = devs.every(isAdult => {
  const age = 2024 - isAdult.year
  return age >= 19
})
...
// Enter your solution code here:
let totalYearsLived = 0;
totalYearsLived = inventors.reduce((yearsPassed, inventor) => {
  const lifespan = inventor.passed - inventor.year;
  return yearsPassed + lifespan;
}, 0);
...
let travelMethodCounts = {};
travelMethodCounts = travelMethods.reduce((vehicle, travelMethod) => {
  const travelMethodTally = vehicle[travelMethod];
  travelMethodTally
    ? vehicle[travelMethod]++
    : (vehicle[travelMethod] = 1);
